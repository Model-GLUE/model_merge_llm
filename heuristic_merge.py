            ...
            if init_sim == -1:
                ...
                if metric == 'dissim':
                    ...
                    if sim > init_sim:
                        ...
                        init_sim = sim
            if args.verbose:
                print(f"[log] {metric} of {current_model} {model} : {sim}")
    elif metric == 'acc':
...
def merge_eval(cfg, args):
    model2eval, tokenizer = run_merge(cfg, options=MergeOptions(), base_model=args.base_model)
    if model2eval:
        model2eval.to('cuda')

    res_acc = evaluate_model(model2eval, tokenizer, bigcode_cfg_path='./files/generation_config.json', task_list=args.task_list)
    del model2eval 
    del tokenizer
    return res_acc
...
if __name__ == "__main__":
    ...
    parser.add_argument("--models_acc_cache_file", type=str, default='./files/model_acc.csv')
    ...
    if args.metric == "acc":
        model_list = get_order_of_models(args.model_list, args.task_list, descending=True, cache_file=args.models_acc_cache_file)

    current_model = args.init_model = model_list[0]
    best_result_in_all_round = evaluate_model(None, None, bigcode_cfg_path='./files/generation_config.json', task_list=args.task_list, model_path=current_model)
